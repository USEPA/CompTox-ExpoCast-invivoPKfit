...
```{r Evaluating-Predictions}
my_data <- get_data(my_pk_000j)

# A quick evaluation of where the data is in terms of highest vs lowest concentration
my_data %>%
  group_by(Chemical, Species) %>%
  mutate(normConc = (Conc - min(Conc))/(max(Conc) - min(Conc))) %>%
  ggplot(aes(x = normConc)) +
  geom_histogram()


my_preds <- bind_rows(list(
  inner_join(predict(my_pk_000p, newdata = my_data, by_timepoint = TRUE) %>%
                         mutate(Option = "000p"), 
                       get_winning_model(my_pk_000p)),
  inner_join(predict(my_pk_000j, newdata = my_data, by_timepoint = TRUE) %>%
                         mutate(Option = "000j"), 
                       get_winning_model(my_pk_000j)),
  inner_join(predict(my_pk_100p, newdata = my_data, by_timepoint = TRUE) %>%
                         mutate(Option = "100p"), 
                       get_winning_model(my_pk_100p)),
  inner_join(predict(my_pk_100j, newdata = my_data, by_timepoint = TRUE) %>%
                         mutate(Option = "100j"), 
                       get_winning_model(my_pk_100j)),
  inner_join(predict(my_pk_111p, newdata = my_data, by_timepoint = TRUE) %>%
                         mutate(Option = "111p"), 
                       get_winning_model(my_pk_111p)),
  inner_join(predict(my_pk_111j, newdata = my_data, by_timepoint = TRUE) %>%
                         mutate(Option = "111j"), 
                       get_winning_model(my_pk_111j))
))

my_preds %>%
  group_by(Chemical, Species) %>%
  mutate(normPred = (Conc_est - min(Conc_est))/(max(Conc_est) - min(Conc_est))) %>%
  ggplot(aes(x = normPred, color = Option)) +
  geom_freqpoly()
...
data_pl <- my_data %>%
  ...
  distinct(Chemical, Chemical_Name, Species,
           Reference, Route, Media, Dose, Conc, Time) %>%
  group_by(Chemical, Chemical_Name, Species) %>%
      ...
      \(x) {
        ...
        p + geom_point(size = 0.7) +
          labs(title = Chemical_Name,
               subtitle = paste(Chemical, Species,
                                sep = ", ")) +
          # facet_wrap(~Route + Media + Dose,scales = "free_y") +
          theme_bw() +
...
pred_pl <- my_preds %>%
  ...
  mutate(pred_plot = purrr::map(predictions,
                                \(x) {
                                  p <- geom_point(data = x,
                                                 mapping = aes(
                                                x = Time,
...
combined_pl <- left_join(data_pl, pred_pl) %>%
  mutate(final_plot = purrr::map2(data_plot, pred_plot,
                                  \(x, y) { 
                                    x + y +
...
pdf(paste0(Sys.getenv("FIG_DIR"),
           "VariousPredictions_BestFits.pdf"),
    width = 10, height = 10, bg = "white",
    onefile = TRUE)
for (i in 1:nrow(combined_pl)) {
  print(combined_pl$final_plot[[i]])
}
dev.off()


comp_preds <- bind_rows(list(
  inner_join(predict(my_pk_000p, newdata = my_data,
                     use_scale_conc = FALSE, by_timepoint = FALSE) %>%
                         mutate(Option = "000p"), 
                       get_winning_model(my_pk_000p)),
  inner_join(predict(my_pk_000j, newdata = my_data,
                     use_scale_conc = FALSE, by_timepoint = FALSE) %>%
                         mutate(Option = "000j"), 
                       get_winning_model(my_pk_000j)),
  inner_join(predict(my_pk_100p, newdata = my_data,
                     use_scale_conc = FALSE, by_timepoint = FALSE) %>%
                         mutate(Option = "100p"), 
                       get_winning_model(my_pk_100p)),
  inner_join(predict(my_pk_100j, newdata = my_data,
                     use_scale_conc = FALSE, by_timepoint = FALSE) %>%
                         mutate(Option = "100j"), 
                       get_winning_model(my_pk_100j)),
  inner_join(predict(my_pk_111p, newdata = my_data,
                     use_scale_conc = FALSE, by_timepoint = FALSE) %>%
                         mutate(Option = "111p"), 
                       get_winning_model(my_pk_111p)),
  inner_join(predict(my_pk_111j, newdata = my_data,
                     use_scale_conc = FALSE, by_timepoint = FALSE) %>%
                         mutate(Option = "111j"), 
                       get_winning_model(my_pk_111j))
))
...
comp_preds %>%
  group_by(Chemical, Species) %>%
  mutate(normPred = (Conc_est - min(Conc_est))/(max(Conc_est) - min(Conc_est)),
         normConc = (Conc - min(Conc))/(max(Conc) - min(Conc))) %>%
  ggplot(aes(y = normPred, x = normConc,
             color = Option)) +
  geom_point(alpha = 0.3, shape = 1) +
  geom_smooth(method = "lm", linewidth = 0.5) +
  geom_abline() +
  facet_grid(rows = vars(Option),
             cols = vars(Route)) +
  theme_bw()


comp_pl <- comp_preds %>%
  left_join(my_data %>% distinct(Chemical, Chemical_Name) %>%
              mutate(Chemical_Name = case_when(
                Chemical_Name %in% "2,4-d" ~ "2,4-dichlorophenoxyacetic acid",
                Chemical_Name %in% "methylene chloride" ~ "dichloromethane",
                .default = Chemical_Name)),
            relationship = "many-to-many") %>%
  group_by(Chemical, Chemical_Name, Species) %>%
  nest(.key = "data_pred") %>%
  mutate(Rsq_plot = purrr::map(
    data_pred,
    \(x) {
      ggplot(data = x,
             mapping = aes(x = Conc,
        ...
        labs(title = Chemical_Name,
             subtitle = paste(Chemical, Species,
                              sep = ", "),
             x = "Observations, log10",
             y = "Predictions, log10",
             color = "Options") +
        # facet_wrap(~Route + Media + Dose, scales = "free") +
        theme_bw() +
        theme(panel.border = element_rect(color = "black", fill = NA,
                                          linewidth = 1),
              plot.title = element_text(hjust = 0.5, face = "bold"),
...
pdf(paste0(Sys.getenv("FIG_DIR"),
           "logScaled_Rsq_BestFits_ChemSpec.pdf"),
    width = 10, height = 10, bg = "white",
    onefile = TRUE)
for (i in 1:nrow(combined_pl)) {
  print(comp_pl$Rsq_plot[[i]])
}
dev.off()


my_rsq <- bind_rows(list(
  inner_join(rsq(my_pk_000p, use_scale_conc = FALSE,
                 rsq_group = ggplot2::vars(Chemical, Species,
                                           Route, Media, Dose)) %>%
                         mutate(Option = "000p"), 
                       get_winning_model(my_pk_000p)),
  inner_join(rsq(my_pk_000j, use_scale_conc = FALSE,
                 rsq_group = ggplot2::vars(Chemical, Species,
                                           Route, Media, Dose)) %>%
                         mutate(Option = "000j"), 
                       get_winning_model(my_pk_000j)),
  inner_join(rsq(my_pk_100p, use_scale_conc = FALSE,
                 rsq_group = ggplot2::vars(Chemical, Species,
                                           Route, Media, Dose)) %>%
                         mutate(Option = "100p"), 
                       get_winning_model(my_pk_100p)),
  inner_join(rsq(my_pk_100j, use_scale_conc = FALSE,
                 rsq_group = ggplot2::vars(Chemical, Species,
                                           Route, Media, Dose)) %>%
                         mutate(Option = "100j"), 
                       get_winning_model(my_pk_100j)),
  inner_join(rsq(my_pk_111p, use_scale_conc = FALSE,
                 rsq_group = ggplot2::vars(Chemical, Species,
                                           Route, Media, Dose)) %>%
                         mutate(Option = "111p"), 
                       get_winning_model(my_pk_111p)),
  inner_join(rsq(my_pk_111j, use_scale_conc = FALSE,
                 rsq_group = ggplot2::vars(Chemical, Species,
                                           Route, Media, Dose)) %>%
                         mutate(Option = "111j"), 
                       get_winning_model(my_pk_111j))
...
ggplot() +
  ...
  ggbeeswarm::geom_beeswarm(
    data = my_rsq %>% 
      filter(Species %in% c("human", "hamster", "dog"),
             !is.na(Rsq)),
    mapping = aes(
      x = Option,
      y = Rsq
  ...
  labs(title = "Distribution of Rsq per select options",
       subtitle = "(calculated using Chemical + Species + Route + Media + Dose grouping)") +
  theme_bw() +
  theme(
    plot.title = element_text(hjust = 0.5),
    plot.subtitle = element_text(hjust = 0.5),
...
ggsave(paste0(Sys.getenv("FIG_DIR"), "Rsq_betterGroups.png"),
       units = "in",
       height = 8,
       width = 8,
       device = "png",
