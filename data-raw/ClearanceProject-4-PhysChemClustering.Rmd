...
```{r data_loading}
target_chem_properties <- readRDS("data-raw/ctxR_3comp2_cheminfo.rds")
...
tcs_mat <- target_chem_summary %>% 
  select(dtxsid, monoisotopicMass, Affinity.Kd.Med,
         H_donors:Rot_Bonds, logkow, apKa, bpKa, logD_7.4,
         starts_with("CYP")) %>%
  distinct()
...
summary(tcs_mat_z[, 1])
summary(tcs_mat_z[(rownames(tcs_mat_z) %in% target_chems), 1])
...
ggplot(data = pca_df %>% mutate(Target = (Chemical %in% target_chems)) %>%
