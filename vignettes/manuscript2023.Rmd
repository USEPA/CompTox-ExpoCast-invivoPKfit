...
clr_pk <- readRDS(paste0(Sys.getenv("OD_DIR"),
                         "pkfits_feature_clrtest/",
                         "pkfit_111h.rds"))
```
...
get_gof <- function(this_error_model,
  ...
  message(paste0("rmse: ", NROW(this_rmse)))
  
  gof_df <- this_rsq %>%
    inner_join(this_AIC, by = join_by(Chemical, Species, method, model)) %>%
    inner_join(this_rmse, by = join_by(Chemical, Species, method, model)) %>%
