...
quick_stats <- function(.data) {
  ...
  message(paste0("Number of unique studies: ", length(unique(.data$fk_study_id))))
  message(paste0("Number of unique series: ", length(unique(.data$fk_series_id))))
  message(paste0("Number of unique chemicals: ", length(unique(.data$analyte_dtxsid))))
  message(paste0("Unique Dose units: ", paste(unique(.data$dose_level_units_original), collapse = ", ")))
  message(paste0("Unique Concentration units: ", paste(unique(.data$conc_units_original), collapse = ", ")))
...
cvt_df <- sql_query %>%
  mutate(
    ...
    across(all_of(c("time_original",
                    ...
                    "loq")), as.numeric),
    # conc, conc_sd_original, and conc_original have NA/NE/ND values that are coerced to NAs
...
cvt_df <- cvt_df %>%
  mutate(invivPK_loq = if_else(
    ...
    invivPK_loq * 1E-3, invivPK_loq)) # mg/L and ug/mL are same scale
