...
```{r, out.width="60%", fig.align="center", fig.cap="Typical plot from which CvT data is extracted. From Sowa and Steibert (1985)."}
...
## **1c) In-House Curation Workflow**
1) Identify publications with CvT data using PubMed queries  

2) Manually extract CvT data using [WebPlotDigitizer](https://automeris.io/WebPlotDigitizer/)

3) Curate CvT data and metadata using EPA's *.xlsx* template  

4) Screen template using *CvTdbQA*, an R Shiny application that checks template for format and reconstructs plots based on reported CvT data

5) Manually check template for format and data correctness

6) Normalize metadata units using an R script workflow, while also retaining original values 

7) Push template to CvTdb
...
```{r, out.width="30%", fig.align="center", fig.cap="Typical plot from which CvT data is extracted. From Sowa and Steibert (1985)."}
...
```{r, fig.cap = "1-Compartment Model. Adapted from Wambaugh et al. (2018).", fig.align = "center",  out.width = "60%"}
...
```{r, fig.cap = "2-Compartment Model. Adapted from Wambaugh et al. (2018).", fig.align = "center", out.width = "60%"}
...
```{r, warning = FALSE, fig.align = "center", out.width = "60%", fig.cap = "Log-Normal Distribution"}
