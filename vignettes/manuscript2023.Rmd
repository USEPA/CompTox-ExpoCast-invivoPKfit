...
comp_preds <- bind_rows(list(
  inner_join(predict(my_pk_000p, newdata = my_data, by_timepoint = FALSE) %>%
                         mutate(Option = "000p"), 
                       get_winning_model(my_pk_000p)),
  inner_join(predict(my_pk_000j, newdata = my_data, by_timepoint = FALSE) %>%
                         mutate(Option = "000j"), 
                       get_winning_model(my_pk_000j)),
  inner_join(predict(my_pk_100p, newdata = my_data, by_timepoint = FALSE) %>%
                         mutate(Option = "100p"), 
                       get_winning_model(my_pk_100p)),
  inner_join(predict(my_pk_100j, newdata = my_data, by_timepoint = FALSE) %>%
                         mutate(Option = "100j"), 
                       get_winning_model(my_pk_100j)),
  inner_join(predict(my_pk_111p, newdata = my_data, by_timepoint = FALSE) %>%
                         mutate(Option = "111p"), 
                       get_winning_model(my_pk_111p)),
  inner_join(predict(my_pk_111j, newdata = my_data, by_timepoint = FALSE) %>%
                         mutate(Option = "111j"), 
                       get_winning_model(my_pk_111j))
))
