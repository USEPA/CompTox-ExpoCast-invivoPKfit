...
In addition, _L-GFGS-B_ was the optimizer of choice
...
ggsave(paste0(Sys.getenv("FIG_DIR"),
              "ModelPerformance_vDataVariability_2023-10-02.png"),
       height = 3,
       width = 6.5,
       device = "png",
       dpi = 300,
...
lombard_abbr <- lombard_abbr %>%
  mutate(Species = "human", model = "lombardo") %>%
...
sfig4A <- lombardo_hist %>% ggplot(aes(x = log2(Vss), color = Source,
                             group = Source)) +
...
sfig4B <- other_meta %>% left_join(my_tkstats, by = c("DTXSID" = "Chemical")) %>%
  ...
  pivot_longer(cols = c(Fgutabs.tkstats, fbior),
    ...
    scale_color_manual(values = c("black", "green4"),
                       breaks = c("invivoPKfit", "Caco-2 Paper")) +
...
Specify that favorable gains in Cmax RMSE not accompanied by gains in AUC RMSE.
Histogram of Volume of Distributions and half-lifes... other TKstats... Lombardo comparison
- Stratified on species... then chemical class (pharma vs non-pharma, Pharmapendium)
- Classyfier for chemical superclass (talk to Paul) Ranges and Mean for each class...
