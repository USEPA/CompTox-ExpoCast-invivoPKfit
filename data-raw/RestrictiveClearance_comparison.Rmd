...
new_pk <- new_pk + settings_optimx(method = c("bobyqa"))
new_pk <- do_prefit(new_pk)


unique(new_pk$prefit$par_DF$param_name)
new_pk$prefit$par_DF %>% arrange(Chemical, Species) %>% filter(param_name == "Fup") %>%
  View()
...
new_pk$fit %>% unnest(fit) %>% select(!starts_with("sigma_")) %>% View()

saveRDS(new_pk, "data-raw/someCVT_wHTTK_reform_JOINT.rds")
...
get_tkstats(new_pk) %>% 
  ...
  left_join(new_pk$prefit$par_DF %>%
              ...
              pivot_wider(names_from = model,
                          values_from = start)) %>% View("Clearance")
...
my_tkstats <- my_pk %>%
  ...
  left_join(rsq(my_pk)) %>% distinct()
my_tkstats %>% select(-coefs_vector) %>% clipr::write_clip()
#Send the R2 too.
