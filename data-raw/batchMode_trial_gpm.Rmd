...
# A PFAS chemical?
my_predata <- cvt %>%
  dplyr::filter(chemicals_analyzed.dsstox_substance_id %in% "DTXSID3031860")
# All long lived chemicals
my_predata <- cvt %>%
  dplyr::filter(chemicals_analyzed.dsstox_substance_id %in% c(long_lived_chem[3:5],
                                                              "DTXSID0020442"))
do_preprocess(my_pk_001p) %>% get_data() %>% filter(Time.Units != Time_trans.Units) %>%
  pull(Chemical) %>% unique() -> long_lived_chem
...
my_predata <- cvt %>%
  dplyr::filter(chemicals_analyzed.dsstox_substance_id %in% (over1Ref %>%
                                                               pull(Chemical)),
...
my_predata <- cvt %>%
  dplyr::filter(chemicals_analyzed.dsstox_substance_id %in% "DTXSID4023917")
...
{
  start_t <- Sys.time()
  my_pk <- do_fit(my_pk, n_cores = 3)
  round(Sys.time() - start_t, 3) # 5.741min (5 data_groups)
} # 1.639min with only one dataset
# my_pk <- do_fit(my_pk)
{
  start_t <- Sys.time()
  my_pk_time_scaling <- do_fit(my_pk_time_scaling, n_cores = 10)
  round(Sys.time() - start_t, 3) # w/ Time scaling 36mins!
}
...
Here I evaluate the choices of the fits in a streamlined way.
...
evaluate_choices <- function(obj, PJ = "Pooled") {
  ...
  my_df <- left_join(fold_errors_all, r2_df) %>%
    ...
    dplyr::select(!c("model_1comp", "model_2comp", "model_flat"))
...
my_pk_000p <- do_fit(my_pk_000p, n_cores = 8) %>% evaluate_choices() # Done
...
# Run all at once
{
current_pk <- my_pk_111p # Define current evaluated pk object here
  ...
  mutate(Options = "Pooled") %>%
    dplyr::select(Options,
      ...
      mutate(SE_Cmax = (Cmax.tkstats - Cmax.nca)^2,
             SE_AUC_inf = (AUC_inf.tkstats - AUC_inf.nca)^2) %>%
      filter(!is.na(SE_Cmax), !is.na(SE_AUC_inf)) %>%
  dplyr::select(Chemical, Species, Route, Media, method, Options,
                SE_Cmax, SE_AUC_inf) %>%
  distinct() %>%
  group_by(Options, method) %>%
      summarize(RMSE_Cmax = sqrt(mean(SE_Cmax,
...
eval_tkstats(obj = my_pk_111p) %>%
  filter(Chemical == "DTXSID0021125") %>%
    mutate(Options = "Pooled") %>%
    dplyr::select(Options,
                Chemical, Species,
                Route, Media, Reference,
...
readxl::excel_sheets("C:/Users/GPADILLA/OneDrive - Environmental Protection Agency (EPA)/DataSummary_8_18_2023.xlsx") # Predictions Evaluation

preval <- readxl::read_xlsx(paste("C:/Users/GPADILLA",
                                  "OneDrive - Environmental Protection Agency (EPA)",
                                  "DataSummary_8_18_2023.xlsx",
                                  sep = "/"),
                            sheet = "Predictions Evaluation") %>%
  dplyr::select(-model_1comp, -model_2comp, -model_flat) %>%
  distinct()
...
### Plotting for all (2GB file)
pdf(file = paste0(Sys.getenv("FIG_DIR"),
              "DoseNormalized_Joint_AllFits.pdf"))
for (i in 1:nrow(pl)) {
