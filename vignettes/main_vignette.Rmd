...
Now, initialize a `pk` object.  This contains the data, along with a default set of instructions for how to do the model fitting. However, none of the instructions have been carried out yet -- this is just the list of instructions.

```{r}
my_pk <- pk(data = my_data) +
  stat_model(model = "1comp")
```

There are four stages to model fitting.
...
You can get the predicted values using `predict()`
...
And finally, you can compute derived toxicokinetic statistics, such as total clearance rate, half-life, steady-state plasma concentration, etc.
