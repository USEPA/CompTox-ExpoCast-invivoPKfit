...
this_pk <- pk(cvt %>% filter(conc_medium_normalized == "plasma")) +
...
winning_preds <- predict(this_pk) %>%
  semi_join(this_nonflat_winmodel) %>%
...
combined_RMSLE %>%
  ...
  geom_point(aes(y = RMSLE_nonrestrictive), shape = 4, size = 0.5) +
...
```{r combine_tkstats}
# Get the winning model and only include chemicals in target_chems
this_winmodel <- get_winning_model(this_pk) %>%
    filter(Chemical %in% target_chems)
...
this_tkstats <- eval_tkstats(this_pk,
  ...
  rename(CLtot_Fgutabs.tkstats = `CLtot/Fgutabs.tkstats`) %>%
  distinct(Chemical, Species, method, model, Dose, Route, Media,
           CLtot.tkstats, CLtot_Fgutabs.tkstats, halflife.tkstats, Css.tkstats) %>%
...
# 46 chemicals have valid total clearance
# 144 chemicals have valid total clearance/Fgutabs
...
this_css_df <- inner_join(
  ...
  left_join(target_chem_lipinski %>% rename(Chemical = dtxsid)) %>%
  mutate(comparison_group_Css = factor(
    case_when(
      between(Css.tkstats, Css_httk_r, Css_httk_nr) |
        between(Css.tkstats, Css_httk_nr, Css_httk_r) ~ "Between",
      Css.tkstats < Css_httk_r & Css.tkstats < Css_httk_nr ~ "Below",
      Css.tkstats > Css_httk_r & Css.tkstats > Css_httk_nr ~ "Above"),
    levels = c("Above", "Between", "Below")
  ), Chemical = factor(Chemical) %>% fct_reorder(Css_httk_r)
  )
...
this_css_df %>%
  ...
  geom_point(aes(y = Css.tkstats, shape = Route), color = "turquoise4") +
...
ggsave(filename = paste0(Sys.getenv("FIG_DIR"),
                         "RestrictiveClearance_Analysis/",
                         "CURRENT_", "Css_comparison_fullChart.png"),
       bg = "white",
       height = 4, width = 8
)
...
ggsave(filename = paste0(Sys.getenv("FIG_DIR"),
                         "RestrictiveClearance_Analysis/",
                         "CURRENT_", "Css_x_CLint.png"),
       bg = "white",
       height = 4, width = 8
)
```

Now I will try to triangulate the model weights witht the Css estimates.

```{r weighted_Css}
combined_model_weights %>%
  inner_join(this_css_df) %>%
  ggplot(aes(x = fct_reorder(Chemical, Css_httk_r))) +
  geom_point(aes(y = Css_httk_r), shape = 4, size = 0.5) +
  geom_point(aes(y = Css_httk_nr), shape = 4, size = 0.5) +
  geom_linerange(aes(ymin = Css_httk_r,
                     ymax = Css_httk_nr)
