...
given a PK model M(x,t) and a statistical model S(œÉ)

This probability is described by a likelihood function L(Cobs, M, S)

We then use an optimizer function to optimize the values of x and œÉ such that 
L(Cobs, M(x), S(œÉ))

We then use the method of quadrature to estimate uncertainty in the estimates 
of x and œÉ as well as any functions of those parameters (for example, Cmax and 
...
If the observation is that the concentration was below a limit of quantitation (LOQ) we call the observation ‚Äúcensored‚Äù
...
CDF(LOQ, Ì†µÌºá, Ì†µÌºé) =1/2+1/2 Ì†µÌ±íÌ†µÌ±üÌ†µÌ±ì((ln‚Å°Ì†µÌ∞øÌ†µÌ±ÇÌ†µÌ±Ñ‚àíÌ†µÌºá)/(‚àö2 Ì†µÌºé))
...
Dov√¨, V. G., O. Paladino, and A. P. Reverberi. "Some remarks on the use of the inverse hessian matrix of the likelihood function in the estimation of statistical properties of parameters."¬†Applied Mathematics Letters¬†4.1 (1991): 87-90.
...
Sheiner, Lewis B. "Analysis of pharmacokinetic data using parametric models‚Äî1: Regression models." Journal of pharmacokinetics and biopharmaceutics 12.1 (1984): 93-117.
...
```
* RStudio provides a menu ‚ÄòInstall Packages‚Äô under ‚ÄòTools‚Äô tab
