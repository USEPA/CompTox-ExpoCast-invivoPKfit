...
```{r update_eval_kstats}


my_eval <- left_join(my_tkstats_red, my_nca_red) %>%
  filter(is.finite(AUC_inf.tkstats), is.finite(AUC_inf.nca)) %>%
  group_by(!!!my_pk$settings_data_info$nca_group)

my_eval %>%
  dplyr::select(AUC_inf.tkstats, AUC_inf.nca, Cmax.tkstats, Cmax.nca) %>%
  ungroup(Dose) %>%
  summarize(AUC_inf_RMSE = abs(mean(AUC_inf.tkstats) - mean(AUC_inf.nca)),
            Cmax_RMSE = abs(mean(Cmax.tkstats) - mean(AUC_inf.nca)))
