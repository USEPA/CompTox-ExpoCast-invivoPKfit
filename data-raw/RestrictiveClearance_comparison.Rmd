...
this_css_df <- this_css_df %>%
  ...
  ), Chemical = factor(Chemical) %>% forcats::fct_reorder(Css_httk_r)
  ) %>%
  mutate(nearest_clearance = factor(
    case_when(
      between(log2(Css.tkstats),
              log2(Css_httk_r)-1, log2(Css_httk_r)+1) |
        (comparison_group_Css == "Above" &
           log2(Css.tkstats) <= log2(Css_httk_r)+2) ~ "Restrictive",
      between(log2(Css.tkstats),
              log2(Css_httk_nr)-1, log2(Css_httk_nr)+1) |
        comparison_group_Css == "Below" ~ "NonRestrictive",
...
this_tkstats %>%
  ...
  facet_grid(cols = vars(comparison_CLtot),
             scales = "free_x") +
  labs(y = "Total Clearance", x = "Chemical", fill = "Inverse Confidence",
       subtitle = "Estimates within 20% of in vitro modelled predictions") +
  theme_bw() +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        plot.subtitle = element_text(hjust = 0.5),
