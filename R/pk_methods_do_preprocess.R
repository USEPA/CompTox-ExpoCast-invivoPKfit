...
do_preprocess.pk <- function(obj){
  ...
  if(is.null(obj$data_original)){
    ...
    if( !(all(routes %in% obj$settings_preprocess$routes_keep) &
      ...
      stop(paste("do_preprocess.pk(): data contains unsupported media and/or routes.",
    ...
    if(any(sapply(list(time_units,
      ...
      stop(paste("do_preprocess.pk(): data contains multiple units for one or more variables.",
