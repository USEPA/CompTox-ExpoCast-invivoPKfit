...
By chemical, species, reference, medium, dose group, and time point: calculate mean and standard deviation.
...
ggplot(cvt_summarized,
  ...
  ggtitle("Mean vs. SD for N=1 data\ngrouped by chemical, reference, medium, dose, timepoint") 
```
...
$$ \log \textrm{SD} = -0.517 + 0.953 \log \textrm{Mean} $$

When translated to the natural scale (raising 10 to the power of both sides), this becomes

$$ \log_{10} \textrm{SD} = -0.517 + \log_{10} \textrm{Mean}^{0.953} $$
$$ \log_{10} \textrm{SD} = \log_{10}{ \left( 10^{-0.517} \times \textrm{Mean}^{0.953} \right)}  $$

Raising 10 to the power of both sides,

$$ \textrm{SD} = 10^{-0.517} \times \textrm{Mean}^{0.953} $$

or approximately

$$ \textrm{SD} = 0.3 \times \textrm{Mean}^{0.953} $$
