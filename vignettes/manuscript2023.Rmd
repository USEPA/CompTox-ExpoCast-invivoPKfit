...
```{r}
my_pk_111h <- readRDS(paste0(Sys.getenv("OD_DIR"),
                 "mypk_fit_111h.rds"))

my_pk_000h <- readRDS(paste0(Sys.getenv("OD_DIR"),
                 "mypk_fit_000h.rds"))
```
...
rmses_Cmax_AUC <- function(this_error_model,
                             this_dose_norm,
                             this_log10_trans,
                             this_time_scale) {
  dose_indic <- as.numeric(this_dose_norm)
  ...
  suppressMessages({
    RMSE_eval <- eval_tkstats(obj = current_rds) %>%
      mutate(Options = pk_name) %>%
      dplyr::select(
        Options,
        Chemical,
      ...
      filter(
        abs(AUC_infinity.nca / AUC_tlast.nca) < 10,
      ...
      rowwise() %>%
      mutate(
        SE_Cmax = (Cmax.tkstats - Cmax.nca) ^ 2,
        SE_AUC_inf = (AUC_infinity.tkstats - AUC_infinity.nca) ^ 2
      ) %>%
      filter(!is.na(SE_Cmax), !is.na(SE_AUC_inf)) %>%
      group_by(Options, method) %>%
      summarize(
        RMSE_Cmax = sqrt(mean(SE_Cmax, na.rm = FALSE)) %>% signif(digits = 4),
        RMSE_AUC = sqrt(mean(SE_AUC_inf, na.rm = FALSE)) %>% signif(digits = 4)
      )
  })
...
get_gof <- function(this_error_model,
  ...
  suppressMessages({
    ...
    this_rmsle <- fold_errors(current_rds,
                      rmse_group = vars(Chemical, Species)) %>%
      inner_join(winning_model) %>%
      group_by(Chemical, Species, method, model) %>%
      summarize(RMSLE = mean(log10(Fold_Error[which(Fold_Error > 0)])^2, 
                             na.rm = TRUE))
  })
...
tf_tests <- function(this_error_model,
  ...
  out <- out$model_error_twofold_summary %>%
    mutate(Options = pk_name)
...
system.time(cmax_auc_rmse_tbl <- fitopts %>%
              dplyr::rowwise() %>%
              dplyr::mutate(nca_comp = list(
                rmses_Cmax_AUC(this_error_model = error_model,
                                 this_dose_norm = dose_norm,
                                 this_log10_trans = log10_trans,
                                 this_time_scale = time_scale))) %>%
...
writexl::write_xlsx(x = list(
  Fit_Counts = wm_comp,
  AUC_and_Cmax_RMSE_summary = cmax_auc_rmse_tbl,
...
gof_tbl %>%
  select(Options, Chemical, Species, method, RMSLE) %>%
  pivot_wider(names_from = Options, values_from = RMSLE) %>%
  # filter(is.finite(`111h`) & !is.finite(`000h`)) %>%
  select(Chemical, Species, method, `000h`, `111h`) %>%
  View("RMSLE_wide")

gof_tbl %>%
  select(Options, Chemical, Species, method, Rsq) %>%
  pivot_wider(names_from = Options, values_from = Rsq) %>%
  filter(`111h` < 0.25 & `000h` > 0.8) %>%
  select(Chemical, Species, method, `000h`, `111h`) %>%
  View("Rsq_wide")


pl_111h <- plot(my_pk_111h, best_fit = TRUE)
pl_000h <- plot(my_pk_000h, best_fit = TRUE)

{
  interest_chem <- "DTXSID9044297"
  cowplot::plot_grid(
    pl_111h %>% filter(Chemical %in% interest_chem) %>% pull(final_plot) %>% .[[1]],
    pl_000h %>% filter(Chemical %in% interest_chem) %>% pull(final_plot) %>% .[[1]],
    labels = c("111h", "000h"), ncol = 1)
  
}

pl_111h %>% filter(Chemical %in% "DTXSID9020667") %>% pull(observation_plot)



all_preds_prep <- all_preds %>% mutate(ID = as.factor(paste(Chemical, Species,
                                                            error_model, method)),
                                                 .keep = "unused") %>%
  select(ID, Options, Route, Media, Dose, Conc, Conc_est, Time)
...
my_pk <- minimal_pk +
  ...
  settings_optimx(method = c("bobyqa", "L-BFGS-B")) +
