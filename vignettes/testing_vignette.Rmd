...
Next, we want to make sure that the output of the
`get_` family of methods/functions is behaving properly.
These are used in other methods so it's unlikely these will give issues.
...
Some of the more involved methods calculate the derived statistics.
...
The `summary` method gives us a `list` of `data.frames` and outputs
for fitted objects. If an object is not fitted, it will alert the user.

```{r final_summary}
# Try assigning this to a variable as well!
summary(my_pk)
```
