...
I don't fully know what all of this SQL query means, but it filters out various "internal" tables that are not actually part of CvTdb, and shows only the tables that are.
...
this_date <- format(Sys.time(), "%Y_%m_%d")
...
```{r}
tmp[, studies.dose_level_normalized_corrected:=as.numeric(studies.dose_level_normalized)]

tmp[studies.dose_level_original_units %in% "ug/mL" &
      studies.administration_route_normalized %in% "oral",
    studies.dose_level_normalized_corrected:=as.numeric(studies.dose_level_original) * 5 * subjects.weight_kg/1000]

tmp[studies.dose_level_original_units %in% "ug/mL" &
      studies.administration_route_normalized %in% "iv",
    studies.dose_level_normalized_corrected:=as.numeric(studies.dose_level_original) * 2 * subjects.weight_kg/1000]
...
ggplot(data = tmp) +
  geom_point(aes(x = conc_time_values.time_hr,
                 y = conc_time_values.conc,
                 shape = factor(documents_extraction.id),
                 color = as.numeric(studies.dose_level_normalized_corrected))) +
  facet_grid(rows = vars(studies.administration_route_normalized)) +
  scale_y_log10() + 
  annotation_logticks(sides = "l") +
...
ggplot(data = tmp) +
  geom_point(aes(x = conc_time_values.time_hr,
                 y = conc_time_values.conc/as.numeric(studies.dose_level_normalized_corrected),
                 shape = factor(documents_extraction.id),
                 color = as.numeric(studies.dose_level_normalized_corrected))) +
  facet_grid(rows = vars(studies.administration_route_normalized)) +
  scale_y_log10() + 
...
```{r}
cvt_data[, studies.dose_level_normalized_corrected:=as.numeric(studies.dose_level_normalized)]
...
## Correcting dose level units for PCB-126

Upon examining PCB-126, I think the original dose units should be ng/kg rather than mg/kg. If we look at https://cebs.niehs.nih.gov/cebs/study/002-02863-0008-0000-9 and look at the summary data rather than individual animal data, the dose units are listed as "ng/kg." In the rest of the studies at https://cebs.niehs.nih.gov/cebs/test_article/57465-28-8 all of the dosing units are ng/kg. So I think that is what it should be.

```{r}
cvt_data[chemicals_dosed.dsstox_substance_id %in% "DTXSID3032179", studies.dose_level_original := "ng/kg"]
cvt_data[chemicals_dosed.dsstox_substance_id %in% "DTXSID3032179", studies.dose_level_normalized_corrected := as.numeric(studies.dose_level_normalized) * 1e-6] #convert to mg/kg
```
...
cvt_data[series.conc_units_original == "ug/L",
         conc_time_values.conc_sd_normalized := conc_time_values.conc_sd_original * 1e-3]

cvt_data[series.conc_units_original == "ug/mL",
         conc_time_values.conc_sd_normalized := conc_time_values.conc_sd_original]

cvt_data[series.conc_units_original == "mg/L",
         conc_time_values.conc_sd_normalized := conc_time_values.conc_sd_original]

cvt_data[series.conc_units_original == "ug/dL",
         conc_time_values.conc_sd_normalized := conc_time_values.conc_sd_original * 1e-4]

cvt_data[series.conc_units_original == "ng/mL",
         conc_time_values.conc_sd_normalized := conc_time_values.conc_sd_original * 1e-3]

cvt_data[series.conc_units_original == "pg/mL",
         conc_time_values.conc_sd_normalized := conc_time_values.conc_sd_original * 1e-6]
...
To save space, filter CVT data to oral/IV routes and blood/plasma media, because nothing else is implemented right now. Also filter down to cases where administered and analyzed DTXSID are the same.

```{r}
cvt <- cvt_data[studies.administration_route_normalized %in% c("oral", "iv") &
                  series.conc_medium_normalized %in% c("blood", "plasma") &
                  chemicals_dosed.dsstox_substance_id == chemicals_analyzed.dsstox_substance_id]
```
...
Filter out any variables that are all-NA.

```{r}
all_na <- cvt[, sapply(.SD, function(x) all(is.na(x)))]
na_vars <- names(all_na)[all_na %in% TRUE]
cvt[, (na_vars) := NULL]
```

Create a harmonized (all lower case) species column.

```{r}
cvt[, subjects.species_harmonized := tolower(subjects.species)]
```
