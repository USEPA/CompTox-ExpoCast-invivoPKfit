                          ...
                          fit.plot.points = 200)
{
  ...
  data.set[,
           Dose.nominal:=round(mean(Dose), digits = 5),
  ...
  evalfun <- function(model, time, dose, route, reference, media, loq, sigma, params.in) ###added reference, media, and loq
  {
    ...
    out <- analytic_model_fun(params=params.in,
                              dose = dose,
                              times = time,
                              time.units = 'h',
  ...
  grDevices::pdf(paste("plots/", gsub(" ", "", model), "-", gsub(" ", "", mean.type), "-", Sys.Date(), ".pdf",
                       sep = ""))
  ...
  for (this.compound in unique(data.set[, Compound])) { #plot each compound separately
    ...
    for (this.reference in unique(this.compound.data[, Reference])) {
      ...
      for(this.medium in unique(this.reference.data[, Media])) {
        ...
        this.medium.data[, "Value"][is.na(this.medium.data[, "Value"])] <- this.medium.data[, mean(LOQ)]
    ...
    print(this.compound)
    if(this.compound == "p-cresol") {browser()}
    ...
    max.time <- round(max(unique(as.numeric(this.compound.data$Time)),
                          na.rm = T) * 1.25, digits = 1) ### added digits to help with low range of timepoints
    ...
    if(any(merge.table$Data.Analyzed == "Joint Analysis")) {
      ...
      merge.table <- tidyr::separate_rows(merge.table, Reference, sep = ", ")

      merge.table <- dplyr::semi_join(merge.table, data.set, by = c("Dose", "Route", "Media", "Reference"))
