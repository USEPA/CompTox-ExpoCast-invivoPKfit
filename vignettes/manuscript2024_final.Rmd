...
```{r}
my_pk <- readRDS(paste0(Sys.getenv("OD_DIR"), "25November2024mypk_fit_110p.Rds"))
```
...
Define a date prefix to read.

```{r}
read_date <- "25November2024"
```
...
#### Supplemental Table 4: Best-fit coefficients and SDs
```{r}
my_coef_sd <- coef_sd(my_pk, method = "bobyqa") #this will take a few minutes to run

my_coef_sd <- my_coef_sd %>%
  rowwise() %>%
  mutate(alerts = paste(alerts[[1]],
                        collapse = ";"))
#save as Supplemental Table 3
writexl::write_xlsx(x = my_coef_sd,
                    path = paste0(Sys.getenv("FIG_DIR"),
                                  "Manu_Files/",
                                  today,
                                  "_Supp_Table4",
...
#### Supplemental Table 5: TK summary stats

```{r}
writexl::write_xlsx(x = my_tkstats,
                    path =  paste0(Sys.getenv("FIG_DIR"),
                                  "Manu_Files/",
                                  today,
                                  "_Supp_Table5",
...
legendB <- get_legend(panelB_plot)

#arrange plots without legend at first
plAB <- plot_grid(panelA_plot,
                   panelB_plot + theme(legend.position = "none"),
                   ncol = 2,
                   labels = c("A", "B"),
                   axis = "tb", align = "hv", rel_widths = c(1,1))
#now add legend as a third square on the grid
comb_plot <- plot_grid(plAB,
                       legendB,
                       ncol = 2,
                       rel_widths = c(1,0.1))
#save PNG version
...
#### Supp. Fig 9: plots of the four chemicals that were best fit by the null model
