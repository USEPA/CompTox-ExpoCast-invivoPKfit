...
minimal_pk <- pk(data = cvt %>%
                   filter(analyte_dtxsid %in% unique(combined_clearance_df$Chemical)),
                 mapping = ggplot2::aes(
...
rest_pk <- do_prefit(new_pk, restrictive_clearance = TRUE)
rest_pk <- do_fit(rest_pk, n_cores = 6) # Worked!

nonrest_pk <- do_prefit(new_pk, restrictive_clearance = FALSE)
nonrest_pk <- do_fit(nonrest_pk, n_cores = 6) # Worked!

old_pk <- do_prefit(new_pk, restrictive_clearance = NULL)
old_pk <- do_fit(old_pk, n_cores = 6)

# Unfortunately, it seems the optimizer fails 
# Need to investigate why!!
# Are the kelims calculated too far from those estimated from the data?
