...
rmsles_Cmax_AUC <- function(
  ...
  RMSLE_highAUC <- RMSLE_eval %>%
     dplyr::filter(is.finite(AUC_infinity.nca),
                            is.finite(AUC_infinity.tkstats),
                            is.finite(Cmax.nca),
                            is.finite(Cmax.tkstats)) %>%
    dplyr::filter(AUC_infinity.nca > 0,
                  AUC_infinity.tkstats > 0) %>%
    group_by(Options, method, model) %>%
    summarise(
      count_AUCinf_NCA_gt20x_AUCinf_pred =  
             sum((log10(AUC_infinity.tkstats) - log10(AUC_infinity.nca)) > log10(20))
    )
  ...
  RMSLE_badobs <- RMSLE_badobs %>%
    left_join(RMSLE_neg, by = c("Options",
                                    "method",
                                "model")) %>%
      left_join(RMSLE_highAUC, by = c("Options",
                                    "method",
                                    "model")) %>%
        rowwise() %>%
  ...
  RMSLE_eval <- RMSLE_eval %>%
    ...
    dplyr::filter(
      ...
      AUC_infinity.tkstats > 0) %>%
    dplyr::filter(
      (log10(AUC_infinity.tkstats) -
         log10(AUC_infinity.nca)) <= log10(20)
    ) %>%
    rowwise() %>%
    mutate(
...
Can start here and read in from the file that was just generated
```{r read_eval}
#read GOF table
...
Here we prepare our results for the CvTdb.
