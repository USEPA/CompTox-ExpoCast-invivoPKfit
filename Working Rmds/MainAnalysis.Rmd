...
cvt_data <- fread("../inst/ext/cvt_data_2022_12_13_17_09_45.csv")
```

Add LOQ when available.

PMIDs of documents

```{r}
unique(cvt_data[is.na(conc_time_values.conc),
                .(documents_extraction.pmid,
                  documents_extraction.doi,
                  documents_extraction.other_study_identifier,
                  documents_extraction.url)])
```


For PMID = 1233222, the original document has the LOQ as 0.08 ug/mL.

```{r}
cvt_data[documents_extraction.pmid %In% "1233222",
         c("loq_original",
                                                     "loq_original_units")= .(0.08,
                                                                              "ug/mL")]
```
