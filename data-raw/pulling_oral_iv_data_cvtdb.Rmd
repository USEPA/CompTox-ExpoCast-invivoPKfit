...
sql_query <- sql_query %>%
  dplyr::rename_with(~gsub(pattern = "fk_", replacement = "",
                         .x), .cols = contains("fk_"))
...
```{r counting-replicates}
count_query <- paste0(
  "SELECT DISTINCT ",
  "a.id, a.time_hr, a.conc, ",
  "b.id, b.analyte_dtxsid, ",
  "b.test_substance_dtxsid, b.conc_medium_normalized, ",
  "c.id, c.administration_route_normalized, ",
  "c.fk_extraction_document_id, c.dose_level_normalized, ",
  "d.id, d.species ",
...
query_db(query = count_query,
         schema = "cvt") %>%
  select(-id) %>% 
  filter(!is.na(analyte_dtxsid),
         analyte_dtxsid == test_substance_dtxsid,
         conc > 0) %>%
  distinct() %>%
  group_by(time_hr, analyte_dtxsid, conc_medium_normalized,
           administration_route_normalized, species, fk_extraction_document_id,
...
cvt_df <- cvt_df %>%
  filter(!is.na(analyte_dtxsid),
         ...
         !(other_study_identifier %in% c("S0916")),
         !(analyte_name_original %in% "Methemoglobin"),
         is.na(radiolabeled))
...
cvt_df %>%
  filter(conc > 0) %>%
  group_by(time_hr, analyte_dtxsid, conc_medium_normalized,
           administration_route_normalized, species, document_id,
           dose_level_normalized) %>%
  distinct() %>%
  count() %>% 
  ungroup() %>%
  filter(n >= 10) %>% 
  distinct(analyte_dtxsid, conc_medium_normalized,
           administration_route_normalized, species, document_id,
           dose_level_normalized) %>%
  arrange(analyte_dtxsid, species, document_id) %>%
  left_join(cvt_df) %>% 
  View()
...
This filtered and transformed dataset has 14,486 observations.
