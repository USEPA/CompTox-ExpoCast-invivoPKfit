...
```{r}
cvt_sub <- cvt %>% dplyr::group_by(chemicals_analyzed.dsstox_substance_id,
...
Note that you could just skip straight to the last step. If you call `fit()` on a newly-initialized `pk` object, all of the preceding steps will be done automatically. The following code will give you exactly the same results as going through each step individually as above.
...
For example, you can extract the fitted model parameter values using `coef()`. The result will be a list of matrices, whose columns are model parameters, and whose rows correspond to the optimization algorithms used for fitting.

```{r}
coef(my_pk)
```

You can extract the residuals using `residuals()` (for the observations in the original data) using `predict()`. The result is a list of matrices, one for each fitted model, whose columns contain the residuals from the fits by each optimization algorithm.

```{r}
my_resids <- residuals(my_pk)
lapply(my_resids, head)
```

You can get the predicted values (for the time points, doses, and routes in the original data) using `predict()`. The result is a list of matrices, one for each fitted model, whose columns contain the predictions from the fits by each optimization algorithm.

```{r}
my_preds <- predict(my_pk)
lapply(my_preds, head)
```

If instead you want to get predicted values for new data (new timepoints, doses, or routes), you can do this by providing argument `newdata`

```{r}
predict(my_pk, newdata = data.frame(Time = seq(0, 5, by = 0.5),
                                    Time.Units = "hours",
                                    Dose = 1,
                                    Route = "iv",
                                    Media = "plasma",
...
And finally, you can compute derived toxicokinetic statistics, such as total clearance rate, half-life, steady-state plasma concentration, etc., using `get_tkstats()`. (The result is a list, one `data.frame` of TK stats for each model that was fitted.)

```{r}
my_tkstats <- get_tkstats(my_pk)
head(my_tkstats$model_1comp)
```
