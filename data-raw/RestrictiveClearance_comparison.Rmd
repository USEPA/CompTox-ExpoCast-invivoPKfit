...
# Fraction unbound data for target chemicals
...
human_chems <- httk::get_cheminfo(info = "DTXSID",
                                species = "human",
                                model = "3compartment2",
                                median.only = TRUE,
                                physchem.exclude = TRUE,
                                suppress.messages = TRUE)
human_chems <- intersect(human_chems, new.raw.httk.data$DTXSID)

all_hum_rat_fups <- chem.physical_and_invitro.data %>%
  filter(DTXSID %in% human_chems) %>%
...
human_chems_fup <- data.frame(
  Chemical = human_chems,
  fup_restrictive = chem.physical_and_invitro.data$Human.Funbound.plasma[],
                              fup_nonrestrictive = params_nr)
```
...
kd_v_fup_plot <- new.raw.httk.data %>%
  ...
  labs(x = "Binding affinity ratio", y = "Fraction unbound\nin plasma") +
  theme_bw() +
...
chem_rell_label <- total_rel_evaluation_df %>% distinct(Chemical, ll_label) %>%
...
target_chem_summary %>%
  inner_join(chem_rell_label, by = join_by(dtxsid == Chemical)) %>%
  distinct(dtxsid, logkow, TPSA, Rot_Bonds, ll_label) %>% 
  pivot_longer(cols = c(logkow, TPSA),
               names_to = "Parameter", values_to = "Value") %>%
  left_join(fup_df, by = join_by(dtxsid == Chemical)) %>%
  ggplot(aes(x = Value, y = Funbound.plasma)) +
  labs(x = "", y = "fup") +
...
httk_AIC <- bind_rows(restrictive_ll,
                      nonrestrictive_ll,
                      AIC(this_pk) %>% filter(model == "model_flat") %>%
                        filter_targets())
...
httk_rel_ll %>%
  count(model_flat > 95, model_3comp2_nonrestrictive > 95,
        model_3comp2_restrictive > 95)

httk_rel_ll %>%
  select(!minAIC) %>%
  mutate(Chemical = forcats::fct_reorder2(Chemical,
                                          model_3comp2_nonrestrictive,
                                          model_flat,
                                          .fun = mean)) %>%
  pivot_longer(cols = starts_with("model_"),
               names_to = "model", values_to = "relative_likelihood") %>%
  ggplot(aes(x = Chemical, y = model)) +
  geom_tile(aes(fill = relative_likelihood)) +
  scale_fill_viridis_b(option = "cividis", breaks = c(0, 50, 80, 100)) +
  theme_bw() +
  theme(axis.text.x = element_blank(),
...
total_rel_evaluation_df <- httk_rel_ll %>%
  inner_join(this_evaluated_df) %>%
  inner_join(this_param_df %>% distinct(Chemical, forced_human_values)) %>%
  distinct()

total_rel_evaluation_df %>%
  distinct(Chemical, Species, Css.tkstats, Css_httk_nr, Css_httk_r, Css_clear,
         ll_label) %>%
  filter(ll_label %in% c("Nonrestrictive", "Restrictive", "Undetermined")) %>%
...
total_rel_evaluation_df %>%
  distinct(Chemical, Species, Css_httk_r,
           halflife.tkstats, restrictive_halflife,
           nonrestrictive_halflife, halflife_clear,
           ll_label) %>%
    filter(ll_label %in% c("Nonrestrictive", "Restrictive", "Undetermined")) %>%
  ggplot(aes(x = forcats::fct_reorder(Chemical, restrictive_halflife))) +
  geom_pointrange(aes(y = halflife.tkstats, 
                      ymin = nonrestrictive_halflife,
                      ymax = restrictive_halflife),
                  size = 0.1) +
  facet_grid(cols = vars(ll_label), space = "free_x", scales = "free_x") +
  scale_y_log10() +
  labs(x = "Chemicals", y = "halflife") +
  theme_bw() +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        strip.text = element_blank(),
