...
```{r data_loading}
# TODO: Adjust this to load in new prediction files
# Adjust rest of document as well!
...
```{r venn_diagrams}
member_list <- list_to_matrix(list(
  CvTdb = cvt_plasma_chems,
  httk_human = human_chems,
  httk_rat = rat_chems
)) |> make_comb_mat()
...
png(
  paste0(
    ...
    "CURRENT_", "httk_vs_CvT_UpsetPlot.png"
  ),
  bg = "white",
...
ComplexHeatmap::UpSet(
  member_list,
  set_order = c("CvTdb", "httk_rat", "httk_human"),
  comb_order = order(comb_size(member_list), decreasing = TRUE),
...
combined_RMSLE <- combined_preds %>%
  ...
  summarize(
    RMSLE_restrictive = sqrt(mean(restrictive_SLE, na.rm = TRUE)),
...
combined_RMSLE %>%
  ungroup() %>%
  pivot_longer(
    cols = starts_with("RMSLE_"),
    names_to = "Source",
    values_to = "RMSLE"
  ) %>%
  mutate(Source = factor(Source,
    levels = c(
      "RMSLE_nonrestrictive",
      "RMSLE_invivoPKfit",
      "RMSLE_restrictive"
    )
  )) %>%
...
this_pk <- this_pk %>% fit_sigma.pk(
  preds = rpr_httk_preds,
  pred_col = "httk_Preds_restrictive"
)
this_pk <- this_pk %>% fit_sigma.pk(
  preds = rpr_httk_preds,
  pred_col = "httk_Preds_nonrestrictive"
)

restrictive_ll <- this_pk$ext_fits.httk_Preds_restrictive$sigma_AIC

nonrestrictive_ll <- this_pk$ext_fits.httk_Preds_nonrestrictive$sigma_AIC

# Get NULL models and winning models AICs (call them empirical model)
invivoPKfit_AIC <- bind_rows(
  this_AIC %>% semi_join(this_winmodel) %>%
    mutate(model = "model_empirical"),
  this_AIC %>% filter(model == "model_flat")
) %>% distinct()

httk_AIC <- bind_rows(
  restrictive_ll %>% mutate(model = "model_3comp2_restrictive"),
  nonrestrictive_ll %>% mutate(model = "model_3comp2_nonrestrictive"),
  invivoPKfit_AIC
)
...
httk_rel_ll <- httk_AIC %>%
  ...
  filter(!is.na(model_3comp2_nonrestrictive)) %>%
  rowwise() %>%
  mutate(minAIC = min(c_across(starts_with("model_")),
    na.rm = TRUE
  )) %>%
  ungroup() %>%
  ...
  mutate(ll_label = factor(
    case_when(
      model_flat > 95 ~ "Undetermined",
      model_3comp2_nonrestrictive > 50 &
        model_3comp2_restrictive > 50 ~ "Undetermined (non-flat)",
      model_3comp2_nonrestrictive > 50 ~ "Nonrestrictive",
      model_3comp2_restrictive > 50 ~ "Restrictive"
    )
  ))

httk_rel_ll %>%
  select(Chemical, Species, starts_with("model_"), minAIC) %>%
  mutate(Chemical = forcats::fct_reorder2(
    Chemical,
    model_flat,
    minAIC,
    mean
  ...
  pivot_longer(
    ...
    names_to = "model", values_to = "relative_ll"
  ) %>%
  mutate(
    source = ifelse(grepl("_3comp2_", model), "httk", "invivoPKfit")
  ) %>%
  ggplot(aes(x = Chemical, y = relative_ll, color = model, shape = source)) +
  geom_point(size = 0.75, alpha = 0.9) +
  scale_color_manual(
    values = c("#33a02c", "#cc88ff", "#7b3294", "orange"),
    breaks = c(
      "model_empirical",
      "model_3comp2_restrictive",
      "model_3comp2_nonrestrictive",
      "model_flat"
    )
  ) +
  scale_shape_manual(values = c(2, 3)) +
  labs(
    title = "Relative likelihoods",
    subtitle = "(rat parameterized with human CLint and fup values)",
    y = "More Likely Model >>>",
    color = ""
  ) +
  theme_bw() +
...
css_pl <- this_rel_evaluated_df %>%
  distinct(
    Chemical, Species, Css.tkstats, Css_httk_nr, Css_httk_r,
    ll_label
  ) %>%
  ggplot(aes(x = Chemical)) +
...
cltot_pl <- this_rel_evaluated_df %>%
  distinct(
    Chemical, Species, CLtot_Fgutabs.tkstats,
...
combined_function_list <- union(
  all_chem_functions,
  target_chem_functions
) # 25 total
...
pre_enrichment_mat <- inner_join(
  ...
  janitor::clean_names() %>%
  mutate(
    n_total_targets = length(target_chems_human),
...
red_assay_res %>%
  distinct(
    DTXSID, Affinity.Kd.Med, Affinity.Kd.Low, Affinity.Kd.High,
    Fit
  ) %>%
  na.omit() %>%
...
all_hum_rat_fups <- chem.physical_and_invitro.data %>%
  ...
  mutate(
    Rat.Funbound.plasma = as.numeric(str_extract(
      Rat.Funbound.plasma,
      "^([^,]+)"
    )),
    Human.Funbound.plasma = as.numeric(str_extract(
...
combined_model_weights <- combined_RMSLE %>%
  ...
  ungroup() %>%
  mutate(
    gamma_weight = 100 * RMSLE_invivoPKfit / relative_likelihood,
...
rph_httk_clearance_df %>%
  ...
  rename(
    "No difference" = "Css_httk_r == Css_httk_nr",
...
rph_httk_clearance_df %>%
  ...
  labs(
    title = "Css differences between restrictive & nonrestrictive clearance",
    y = "Css\n(mg/L)", x = "Chemicals"
  ) +
  theme_bw() +
  theme(
...
rph_httk_clearance_df %>%
  ...
  labs(
    title = paste0(
      "Relative Css differences between\n",
...
this_tkstats %>%
  ...
  mutate(comparison_CLtot = factor(
    case_when(
      between(
        log2(CLtot.tkstats), log2(CLtot_restrictive) - 1,
        log2(CLtot_restrictive) + 1
      ) ~ "Restrictive",
      between(
        log2(CLtot.tkstats), log2(CLtot_nonrestrictive) - 1,
        log2(CLtot_nonrestrictive) + 1
      ) ~ "Nonrestrictive",
      between(
        CLtot.tkstats,
  ...
  labs(
    y = "Total Clearance", x = "Chemical", fill = "Inverse Confidence",
...
this_fupZ_df <- rph_param_df %>%
  select(Chemical, Funbound.plasma) %>%
  mutate(
    Fup_Z = as.vector(scale(sqrt(Funbound.plasma))),
    .keep = "unused"
  )
```
...
this_coefsd %>%
  filter(param_name == "Clint", Chemical %in% clear_chem_levels) %>%
  ggplot(aes(
    x = factor(Chemical, levels = clear_chem_levels),
    y = param_value,
    color = model
  )) +
  geom_point(alpha = 0.5) +
