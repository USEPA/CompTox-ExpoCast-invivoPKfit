...
member_list <- list(
  httk_gas_pbtk = unique(get_cheminfo(info = "DTXSID",
                                ...
                                suppress.messages = TRUE)) %>% 
    suppressWarnings(),
  cvt = cvt %>% filter(species %in% c("rat")) %>% pull(analyte_dtxsid)
)
...
ggsave(filename = paste0(Sys.getenv("FIG_DIR"),
                         "RestrictiveClearance_Analysis/",
                         today, "httk_vs_CvT_VennDiagram_ratOnly.png"),
       bg = "white",
       height = 4, width = 6
)
...
my_tkstats <- eval_tkstats(my_pk, finite_only = FALSE) %>%
  ...
  semi_join(get_winning_model(my_pk)) %>%
...
httk_preds_rat <- get_httk_preds(rat_pars_rat, my_pk, species = "rat") 
...
rat_df <- httk_preds_rat %>%
  left_join(predict(my_pk)) %>%
  semi_join(get_winning_model(my_pk))

rat_df <- ggplot(mapping = aes(
    x = Conc_est,
    y = httk_Preds,
    color = interaction(Route, Media)
  )) +
  geom_point() +
  geom_line(mapping = aes(
    group = interaction(Chemical, Media, Route, Reference)
  )
  ) +
  geom_abline(slope = 1, intercept = 0, color = "black") +
  coord_equal(ylim = c(0, 200), xlim = c(0, 200))
...
new_pk$fit %>%
  ...
  geom_point(data = new_pk$fit %>%
               filter(param_name %in% "Fup",
                      model %in% "model_1comp_fup",
                      Chemical %in% combined_clearance_df$Chemical),
