...
my_preds %>%
  ...
  mutate(pass = ifelse(Pred_Obs >= 0.5 & Pred_Obs <= 2, TRUE, FALSE)) %>%
  pull(pass) %>% table() %>% {.[[2]]/(.[[1]] + .[[2]])}
  ggplot(aes(x = Pred_Obs)) +
  geom_histogram(binwidth = 0.5,
                 fill = "grey5") +
  scale_x_log10(limits = c(NA, 1E8)) +
  expand_limits(y = 0) +
  geom_vline(xintercept = c(0.5, 2), color = "red3", linetype = "dashed") +
...
# 44.66% Are above two-fold
# 17.78%  Are below two-fold
# 37.55% Are within twofold
...
pl <- plot(my_pk, n_interp = 12, use_scale_conc = FALSE, best_fit = FALSE)
pdf(file = paste0(Sys.getenv("FIG_DIR"),
