...
my_predata <- cvt %>%
  dplyr::filter(chemicals_analyzed.dsstox_substance_id %in% c("DTXSID0020442",
                                                              ...
                                                              "DTXSID2021103"))
my_predata <- cvt %>%
  dplyr::filter(chemicals_analyzed.dsstox_substance_id %in% "DTXSID1043881")
...
my_tkstats <- inner_join(winmodels, my_tkstats) %>%
  dplyr::select(-TKstats, -Reference) %>% distinct()
...
my_eval_joint <- eval_tkstats(my_pk)
...
total_eval %>%
  ...
  filter(!(ratio == 1)) %>% pull(Chemical) %>% unique()
  View()
