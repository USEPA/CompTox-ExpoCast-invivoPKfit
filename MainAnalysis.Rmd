...
run_timestamp_1comp <- format(Sys.time(), "%Y_%m_%d_%H_%M_%S")
...
run_timestamp_2comp <- format(Sys.time(), "%Y_%m_%d_%H_%M_%S")
...
run_timestamp_flat <- format(Sys.time(), "%Y_%m_%d_%H_%M_%S")
...
```{r}
DF <- preprocess_data(data.set = cvt.data,
...
pk_output_2comp <- read.csv(paste("inst/ext/PK_fit_table_2comp_",run_timestamp_1comp,".csv",sep=""))
pk_output_flat <- read.csv(paste("inst/ext/PK_fit_table_flat.",run_timestamp_2comp,".csv",sep=""))
pk_output_1comp <- read.csv(paste("inst/ext/PK_fit_table_1comp_",run_timestamp_flat,".csv",sep=""))
...
pk_fit_win <- unique(pk_fit[, .(Analysis_Type, DTXSID, Species, model, AIC)] )
```
...
plot_table <- unique(pk_fit[, .(DTXSID, Species, Analysis_Type, Reference)])
plot_table <- plot_table[Analysis_Type %in% "Joint"]

tmp <- mapply(plot_fit,
              DTXSID_in = plot_table$DTXSID,
                     Species_in = plot_table$Species,
                     Analysis_Type_in = plot_table$Analysis_Type,
                     Reference_in = plot_table$Reference,
              MoreArgs = list(
