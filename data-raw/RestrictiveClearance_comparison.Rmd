...
my_tkstats <- eval_tkstats(my_pk) %>%
  ...
  filter(
    Species %in% "rat",
    Chemical %in% rat_pars_rat$clearance_df$Chemical,
...
# Combine these two
combined_clearance_df <- my_tkstats %>% inner_join(rat_pars_rat$clearance_df)
```
...
fup_df %>%
  ...
  geom_histogram(
    data = fup_df %>%
      filter(Chemical %in% rat_pars_rat$param_df$Chemical),
    # aes(y = after_stat(density)),
    fill = NA,
...
minimal_pk <- pk(data = cvt %>%
                   filter(analyte_dtxsid %in% rat_pars_rat$param_df$Chemical,
                          species %in% "rat"))
...
new_pk$fit %>%
  ...
  mutate(direction = ifelse(abs(estimate - start) < 0.1,
                            ...
                            ifelse(estimate > start,
                                   "increase",
                                   "decrease"))) %>%
  filter(Chemical %in% clear_chem_levels, direction %in% "similar (less than 10% difference)")
...
ggsave(filename = paste0(
  Sys.getenv("FIG_DIR"), "RestrictiveClearance_Analysis/",
  today, "fup_optimization_generalFits.png"),
  height = 5, width = 7)
