...
winning_model <- get_winning_model(my_pk)
...
myAIC <- AIC(my_pk, method = "L-BFGS-B") %>% inner_join(winning_model)
myRMSE <- rmse(my_pk, method = "L-BFGS-B") %>% inner_join(winning_model)
...
my_df <- left_join(fold_errors_all, r2_df) %>% inner_join(myAIC) %>% inner_join(myRMSE)
...
ggExtra::ggMarginal(mypl, groupFill = TRUE,
                    type = "histogram",
...
ggplot(data = rmse(my_pk, method = "L-BFGS-B",
                   rmse_group = vars(Route, Media, Dose),
                   use_scale_conc = FALSE) %>%
         inner_join(winning_model) %>%
         mutate(RMSE = ifelse(RMSE == 0, 1E-6, RMSE)),
               aes(
                 x = log10(RMSE),
                 color = model,
                 fill = model
  ...
  # geom_freqpoly(aes(groups = paste(Chemical, Species)),
  #               color = "black",
  #               bins = 5) +
