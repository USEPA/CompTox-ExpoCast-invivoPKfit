...
post_flat <- postprocess_data(PK_fit = PK.fit.table.flat,
                               model = "flat")

write.csv(post_flat,
          paste("../inst/ext/PK_fit_postprocess_flat_",
                                    timestamp,
                                    ".csv",
                                    sep=""),
          row.names = FALSE)

rm(PK.fit.table.flat, post_flat)
```
...
DTXSID1022265 (Alachlor) is an example of one chemical where there is only one Reference ID, but the PO and IV data are on totally different scales -- IV concentration/dose ranges from 1e-3 to 1e-1, but PO concentration/dose ranges from 1e-5 to 1e-4. Now that we are fitting on natural scale, this means that with reference-specific sigma, the curve really only fits the IV data, because sigma is so large compared to the PO data scale. This results in a too-small Fgutabs estimate that does not fit the peak very well. But with "study" specific sigma -- chemical + species + reference + route + media -- the PO sigma can be on a different order than the IV sigma, and is.

Another possible thing to try is to go back to fitting on the log scale. I had switched to the natural scale because I wasn't sure how to derive the "summary" parameterization of the residual distribution for the log scale. But I could try again.
