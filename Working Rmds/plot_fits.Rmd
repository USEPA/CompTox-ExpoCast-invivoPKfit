...
Keep parameters only -- that is, drop the calculated quantities like tpeak, Cpeak, Css, total clearance, etc.
...
Merge the parameter information with the original concentration-time data.
...
```{r}
DT_1comp <- fread(paste0("../inst/ext/", "PK_fit_table_1comp_", timestamp, ".csv",sep=""))
DT_2comp <- fread(paste0("../inst/ext/", "PK_fit_table_2comp_", timestamp, ".csv",sep=""))
DT_flat <- fread(paste0("../inst/ext/", "PK_fit_table_flat_", timestamp, ".csv",sep=""))
...
DT_all <- rbindlist(list(DT_1comp, DT_2comp, DT_flat),
                    use.names = TRUE,
                    fill = TRUE)
  DT_all <- DT_all[grepl(x = param_name,
                 pattern = "sigma"),
           .SD, .SDcols= c(idcols, "model", "param_name", "Fitted mean", "Fitted std dev")]
  DT_melt <- dcast(DT_all,
                   formula = Analysis_Type + DTXSID + Species + References.Analyzed + Studies.Analyzed + param_name ~ model,
                   value.var = "Fitted mean")
  setnames(DT_melt,
           unique(DT_all$model),
           paste0("sigma.", unique(DT_all$model)))
  DT_melt[param_name %in% "sigma", sigma_studyID := Studies.Analyzed]
  DT_melt[!(param_name %in% "sigma"), sigma_studyID := gsub(x = param_name,
                                                            pattern = "sigma_study_",
                                                            replacement = "",
                                                            fixed = TRUE)]
  DT_melt_sd <- dcast(DT_all,
                   formula = Analysis_Type + DTXSID + Species + References.Analyzed + Studies.Analyzed + param_name ~ model,
                   value.var = "Fitted std dev")
   setnames(DT_melt_sd,
           unique(DT_all$model),
           paste0("sigma_sd.", unique(DT_all$model)))
 DT_melt_sd[param_name %in% "sigma", sigma_studyID := Studies.Analyzed]
  DT_melt_sd[!(param_name %in% "sigma"), sigma_studyID := gsub(x = param_name,
                                                            pattern = "sigma_study_",
                                                            replacement = "",
                                                            fixed = TRUE)]
  
  DT_melt_all <- merge(DT_melt, DT_melt_sd, by = intersect(names(DT_melt),
                                                           names(DT_melt_sd)))
...
```{r}
cvt_sd <- cvt_fit[, .(sd_Value_Dose = sd(Value_Dose, na.rm = TRUE)),
        by = Study_orig]
```

Merge SD of data with fitted SDs and compare

```{r}
sd_table <- cvt_sd[DT_melt_all,
...
```
Do the same, but faceting by dose.
...
Do non-dose-faceted, but log scale.
