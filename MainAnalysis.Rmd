...
A couple of compounds had no CAS in CvTdb. Upon double checking, they should not have been added. So remove them. (CLR 2022-11-15: it doesn't look like there are any of these)
...
if fk_reference_document is NULL, make fk_reference_document equal fk_extraction document. (CLR 2022-11-15: it doesn't look like there are any of these)
...
Set a timestamp: today's date. (I am changing this to YYYYMMDD format -- better sorting by filename. -- CLR)

```{r, set_file_timestamp, eval = TRUE}
THIS.DATE <- "20221115"
```

Run a one-compartment fit on the data, and write out the results.
...
```{r, run_invivoPKfit_2comp, eval = FALSE}
...
```{r, run_invivoPKfit_flat, eval = FALSE}
### do flat fit
...
pk_output_2comp$Vdist <- pk_output_2comp$V1*(pk_output_2comp$k12+pk_output_2comp$k21)/pk_output_2comp$k21
