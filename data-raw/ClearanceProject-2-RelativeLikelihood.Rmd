...
this_rel_evaluated_df <- this_css_df %>%
  ...
  inner_join(data.frame(this_clint_df)) %>%
  inner_join(this_param_df %>% 
               distinct(Chemical,
                        forced_human_values,
                        Fabsgut.restrictive,
                        Fabsgut.nonrestrictive)) %>%
  mutate(Chemical = forcats::fct_reorder(Chemical, restrictive_halflife)) %>%
  ungroup() %>%
  distinct()
...
cltot_pl <- this_rel_evaluated_df %>%
  ...
  geom_pointrange(aes(y = CLtot_Fgutabs.tkstats,
                      ymin = restrictive_clearance/Fabsgut.restrictive,
                      ymax = nonrestrictive_clearance/Fabsgut.nonrestrictive),
                  size = 0.1, shape = 23) +
  facet_grid(cols = vars(ll_label), space = "free_x", scales = "free_x") +
  scale_y_log10() +
...
all_chem_functions <- functional_uses_all %>%
  filter(DTXSID %in% all_chems, !is.na(`Harmonized Functional Use`)) %>% 
  group_by(`Harmonized Functional Use`) %>% 
  count() %>% arrange(-n) %>% head(20) %>% pull(1)
target_chem_functions <- functional_uses_all %>%
  filter(DTXSID %in% target_chems, !is.na(`Harmonized Functional Use`)) %>% 
  group_by(`Harmonized Functional Use`) %>% 
  count() %>% arrange(-n) %>% head(20) %>% pull(1)
...
pre_enrichment_mat <- inner_join(
  functional_uses_all %>%
    filter(
      DTXSID %in% all_chems, !(DTXSID %in% target_chems), # all non-targets
  ...
  functional_uses_all %>%
    filter(
      DTXSID %in% target_chems, 
      `Harmonized Functional Use` %in% combined_function_list
    ) %>% distinct(DTXSID, `Harmonized Functional Use`) %>%
    count(`Harmonized Functional Use`, name = "n_targets")
) %>%
  janitor::clean_names() %>%
  mutate(n_total_targets = length(target_chems),
         n_total_nontargets = length(all_chems) - n_total_targets,
...
# Check to see how much of the full dataset is available
length(intersect(unique(new.raw.httk.data$DTXSID), full_list))

focus_chems <- union(new.raw.httk.data$DTXSID, target_chems)

all_chem_properties %>%
  filter(propertyId %in% c("logkow-octanol-water",
                     "pka-acidic-apparent",
                     "pka-basic-apparent",
                     "water-solubility",
                     "logkoa-octanol-air",
