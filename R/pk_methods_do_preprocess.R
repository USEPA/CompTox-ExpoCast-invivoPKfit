  ...
  if (is.null(obj$data_original)) {
    ...
    if (!obj$settings_preprocess$suppress.messages) {
      ...
      message(paste0(paste(
        paste(nrow(data), "concentration vs. time observations loaded."),
        paste0(
          "Number of unique data groups ",
          "(unique combinations of ",
          paste(sapply(data_group, rlang::as_label), collapse = ", "),
          "): ",
          n_grps
        ),
        sep = "\n"
    ...
    if (obj$settings_preprocess$impute_sd %in% TRUE) {
      if (any((data$N_Subjects > 1) %in% TRUE & is.na(data$Value_SD))) {
        if (!obj$settings_preprocess$suppress.messages) {
          ...
          message(
            paste0(
              "Estimating missing concentration SDs (for data points with N_Subjects > 1) as ",
    ...
    if (any((data$N_Subjects > 1) %in% TRUE &
      ...
      if (!obj$settings_preprocess$suppress.messages) {
        message(
          paste0(
            "Excluding remaining observations with N_Subjects > 1 where reported SD is NA. ",
    ...
    if (any((data$N_Subjects > 1) %in% TRUE & is.na(data$Value))) {
      if (!obj$settings_preprocess$suppress.messages) {
        message(
          paste0(
            "Excluding observations with N_Subjects > 1 where reported Value is NA (because log-likelihood for non-detect multi-subject observations has not been implemented). ",
            sum((data$N_Subjects > 1) %in% TRUE &
                  is.na(data$Value)),
            " observations will be excluded.\n"
          )
      ...
      data$exclude_reason <- ifelse((data$N_Subjects > 1) %in% TRUE &
                                      is.na(data$Value),
                                    paste2(data$exclude_reason, "N_Subjects > 1 and Value is NA", sep = "; "),
                                    data$exclude_reason
      )
    ...
    if (!obj$settings_preprocess$suppress.messages) {
      ...
      if ("Conc_trans" %in% names(data)) {
        message(
          "Warning: variable `Conc_trans` already exists in the data. It will be overwritten!\n"
        )
      }
      if ("Conc_SD_trans" %in% names(data)) {
        message(
          "Warning: variable `Conc_SD_trans` already exists in the data. It will be overwritten!\n"
        )
      }
      if ("Conc_trans.Units" %in% names(data)) {
        message(
          "Warning: variable `Conc_trans.Units` already exists in the data. It will be overwritten!\n"
        )
      }
    }
    ...
    #Transform Conc_SD
    data$Conc_SD_trans <- rlang::eval_tidy(obj$scales$conc$expr, data = cbind(data, data.frame(.conc = data$Conc_SD)))
