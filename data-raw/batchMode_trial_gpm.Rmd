...
my_pk_111p <- do_data_info(my_pk_111p)
my_nca1 <- get_nca(my_pk_111p)
...
winning_model <- get_winning_model(my_pk_111p)
winning_model %>%
  group_by(method, model) %>%
  count() %>%
  mutate(Options = "111 Pooled") %>%
  clipr::write_clip(col.names = FALSE)

clipr::clear_clip()


# TKstats/Cmax/AUC analysis... combined!
final_tkstats <- eval_tkstats(my_pk_111p) %>%
  mutate(Options = "111 Pooled")
# Will need to evaluate the best model + method + options

# RMSEs for Cmax and AUC_inf
final_tkstats %>%
...
# Other metrics (Rsq, Fold-Error, prediction RMSE)
# May need to change the use_scale_conc defaults
r2_df <- rsq(my_pk_111p, use_scale_conc = FALSE) %>%
  inner_join(winning_model)
myAIC <- AIC(my_pk_111p) %>%
  inner_join(winning_model)
fold_errors_all <- fold_errors(my_pk_111p) %>%
  inner_join(winning_model) %>%
  dplyr::filter(Detect %in% TRUE) %>%
  dplyr::group_by(!!!my_pk_111p$data_group, model, method) %>%
  dplyr::summarize(avg_FoldErr = mean(Fold_Error, na.rm = TRUE),
                median_FoldErr = median(Fold_Error, na.rm = TRUE),
                within_2fold = sum(Fold_Error >= 0.5 & Fold_Error <= 2)/length(Fold_Error))

myRMSE_all <- rmse(my_pk_111p,
               rmse_group = vars(Route, Media, Dose),
               use_scale_conc = FALSE) %>% 
  inner_join(winning_model) %>%
  dplyr::group_by(!!!my_pk_111p$data_group, model, method) %>%
...
my_df <- left_join(fold_errors_all, r2_df) %>%
  ...
  mutate(Options = "111 Pooled", .before = Chemical)
  

my_df %>% clipr::write_clip(col.names = FALSE)

clipr::clear_clip()

hist(my_df$within_2fold)
